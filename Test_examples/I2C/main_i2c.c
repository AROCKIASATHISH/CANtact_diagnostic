...
//#define I2C_MASTER_BOARD
#define I2C_SLAVE_BOARD
...
int main(void)
{
	...
	MX_GPIO_Init();
	led_init();
        #ifdef I2C_MASTER_BOARD
        I2C_Master_init(100);  //100khz
        #elif defined I2C_SLAVE_BOARD
    ...
    for (;;) {
		 ...
		 I2C_Master_tx_packet(&buffer1,I2C_address,sizeof(buffer1),HAL_MAX_DELAY);
		 HAL_Delay(1000); 
        ...
        #elif defined I2C_SLAVE_BOARD
                 uint8_t buffer[10];
		 if(I2C_Slave_rx_packet(&buffer ,10 , HAL_MAX_DELAY)== HAL_OK)
