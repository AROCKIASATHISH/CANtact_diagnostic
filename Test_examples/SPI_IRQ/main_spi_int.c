...
#ifdef SPI_SLAVE_BOARD
#define SPI_Rx_buff_size 20
uint8_t SPI_RxBuffer[SPI_Rx_buff_size];
uint8_t SPI_RX_buff_index = 0;
...
void HAL_SPI_RxCpltCallback(SPI_HandleTypeDef *hspi)
{
  if (hspi->Instance == SPI1)
  {
    ...
    SPI_RxBuffer[SPI_RX_buff_index]=SPI_Rx_IT_Char();
    SPI_RX_buff_index++;

    if(SPI_RX_buff_index == SPI_Rx_buff_size)
     SPI_RX_buff_index = 0;
...
int main(void)
{
    ...
    for (;;) {
       ...
       #elif defined SPI_SLAVE_BOARD
		if(SPI_RX_buff_index != 0)
                {
                        HAL_Delay(100);
		  	if(strstr(SPI_RxBuffer, "led on"))
			     HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1, GPIO_PIN_SET);
			else if(strstr(SPI_RxBuffer, "led off"))
			     HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1, GPIO_PIN_RESET);
                        SPI_RX_buff_index = 0;
                        memset(SPI_RxBuffer,0,SPI_Rx_buff_size);
               }
